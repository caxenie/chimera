...
function [sensory_data, datax_extrapolated] = visualize_results(sensory_data, populations, learning_params, d, sd)
figure;
set(gcf, 'color', 'w');
% sensory data
subplot(4, 1, [1 2]);
...
switch d
    case 0
        switch sd
            ...
            case 2
                imagesc(populations(1).Wcross, [0, max(populations(1).Wcross(:))]); box off; colorbar;
            case 3
                imagesc(rot90(populations(1).Wcross'), [0, max(populations(1).Wcross(:))]); box off; colorbar;
        end
    case 1
